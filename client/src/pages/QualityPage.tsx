...
import { ShieldCheck, CheckCircle2, FileSpreadsheet, Scale, Award, Download } from 'lucide-react';
    ...
    <div className="py-14 bg-steel-darkest min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {tests.map((test, idx) => (
            <div key={idx} className="p-8 rounded-2xl bg-steel-forest/60 border border-steel-rich space-y-3">
              ...
              <h3 className="text-lg font-bold text-white uppercase tracking-tight">{test.title}</h3>
              <p className="text-sm text-zinc-400 leading-relaxed font-normal">{test.desc}</p>
            </div>
          ))}
        </div>

        {/* Sample MTC Card */}
        <div className="p-8 sm:p-12 rounded-3xl bg-steel-forest border border-steel-accent/40 shadow-2xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <ul className="space-y-3 text-sm text-zinc-300 mb-6 font-normal">
            ...
            <div className="p-6 rounded-2xl bg-steel-darkest border border-steel-rich font-mono text-xs text-zinc-300 space-y-3">
              <div className="text-emerald-400 font-bold text-sm border-b border-steel-rich pb-2">
                TYPICAL CHEMICAL & MECHANICAL TEST BENCHMARK
              </div>
              <div className="flex justify-between">
                <span>Grade:</span>
                <span className="text-white font-bold">IS 1786 Fe 500D</span>
              </div>
              <div className="flex justify-between">
                <span>Carbon (C):</span>
                <span className="text-white">0.25% Max (Actual: 0.21%)</span>
              </div>
              <div className="flex justify-between">
                <span>Sulphur (S) + Phos (P):</span>
                <span className="text-white">0.075% Max (Actual: 0.054%)</span>
              </div>
              <div className="flex justify-between">
                <span>Yield Strength:</span>
                <span className="text-white font-bold">Min 500 MPa (Actual: 535 MPa)</span>
              </div>
              <div className="flex justify-between">
                <span>Elongation:</span>
                <span className="text-white font-bold">Min 16.0% (Actual: 18.5%)</span>
              </div>
              <div className="pt-2 text-[10px] text-steel-olive">
